...
import cv2
import numpy as np
#o_jpg = mpimg.imread('lane.jpg')
o_jpg = mpimg.imread('lane.jpg')
o_jpg = cv2.cvtColor(o_jpg,cv2.COLOR_RGB2BGR) #cv2 为BGR 先转换好
...
for line in line_left:
    ...
    if k_mean_difference[np.argmax(k_mean_difference)] > 0.4: # argmax返回最大数的索引
        print(line_left_k[np.argmax(k_mean_difference)])
...
for line in line_right:
    ...
    if k_mean_difference[np.argmax(k_mean_difference)] > 0.1: # argmax返回最大数的索引
        print(line_right_k[np.argmax(k_mean_difference)])
...
#output = cv2.addWeighted(o_jpg, 0.8,mask, 1, 0)
...
#print(o_jpg)
#o_jpg_plot = plt.imshow(jpg_gray)
#plt.show()
#plt.show(o_jpg_plot)
#print(o_jpg_plot)
